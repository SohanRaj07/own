...
if ($_FILES['excel_file']['error'] == 0) {
    ...
    foreach ($worksheet->getRowIterator() as $row) {
        ...
        $query = "INSERT INTO employee_info (name, employee_id, title, email, phone, aadhar_number) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('ssssss', $name, $employeeId, $title, $email, $phone, $aadhar);
